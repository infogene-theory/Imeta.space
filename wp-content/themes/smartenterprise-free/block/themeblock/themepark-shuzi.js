...
registerBlockType('themepark-block/themepark-shuzi', {
        title: '并列数字动态上涨', 
        icon: 'forms', 
        category: 'themepark-block-b2',
	    keywords: ["并列","数字","icon"],
        description:"使用此区块可以输出一个从0上涨的数字模块",
    ...
    attributes: {  
	   ...
	   icon:{type: 'array',default: [{id:Date.now(), text:"", text2:"",icon:"0",iconc:"",icont:"",url:""}]},
		title:{type: 'string'},
		title2:{type: 'string'},
...
s[i]=	el("span",{},str_array[i])
	...
	for (var index=0;index<icon.length;index++){
		...
		console.log(icon[index].svg);
			...
			svgbox=el("div",{className:'shuzi',Style:iconcs+iconfs+iconcs2},icon[index].icon);
		...
		dieletsvg=el( PanelRow, {}, 						
			...
			el( "lable", {Style:"width:100%;"},'数字的颜色',	  el(ColorPalette,{
			colors : [
		...
		conicon[index]=el( PanelBody, { title: '项目'+(index+1), initialOpen: false },
						 
						  	
				icontdbox2,dieletsvg,
						  

									   el( PanelRow,{},
										
			 el(TextControl, {label: '标题',
	...
	var control=el( Fragment, {},el( InspectorControls, {},
		...
		value: props.attributes.blockbtnc}))) 
				...
				el( PanelBody, { title: '增加项目', initialOpen:true },
...
el( PanelBody, { title: '区块设置', initialOpen:false },
	  ...
	  el( PanelRow, {},
		...
		value: props.attributes.iconl})),													  
			  ...
			  el( PanelRow, {}, el( RangeControl,{label: '数字尺寸',min:24,max:100,onChange: function (el) {props.setAttributes({iconf: el })},
  ...
  el( PanelRow, {}, el( RangeControl,{label: '文字描述高度（响应式对齐）',min:0,max:500,onChange: function (el) {props.setAttributes({boxhi: el })},
		...
		value: props.attributes.txtc1}))),
		  el( PanelRow, {}, el( RangeControl,{label: '标题字号',min:12,max:50,onChange: function (el) {props.setAttributes({ txtf1: el })},
		...
		value: props.attributes.txtc2}))),
		  el( PanelRow, {}, el( RangeControl,{label: '描述字号',min:12,max:50,onChange: function (el) {props.setAttributes({ txtf2: el })},
		value: props.attributes.txtf2})),
																								  
															  
		el( PanelRow, {},   
			el( "lable", {Style:"width:100%;"},'数字块背景颜色',	  el(ColorPalette,{
			colors : [
        { name: '灰色', color: '#666666' },
		{ name: '白色', color: '#ffffff' },
        { name: '深蓝', color: '#146ba2' },
		...
		el( PanelRow, {}, el( RangeControl,{label: '数字块背景颜色透明度',min:0,max:100,onChange: function (el) {props.setAttributes({ bc1o: el })},
		value: bc1o})),							   
